        ...
        <div className="bg-white dark:bg-slate-900/50 rounded-2xl border border-gray-200 dark:border-slate-800 overflow-hidden transition-colors">
          <div className="px-4 py-3 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 bg-gray-100 dark:bg-slate-800 rounded-lg flex items-center justify-center text-gray-500 dark:text-slate-400">
                {darkMode
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 dark:text-slate-400">{darkMode ? 'On' : 'Off'}</p>
              </div>
            </div>
            <button
              role="switch"
              ...
              className={`relative flex-shrink-0 w-11 h-6 rounded-full transition-colors duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 ${darkMode ? 'bg-indigo-600' : 'bg-gray-200 dark:bg-slate-700'}`}
            >
        ...
        {isAdmin && (
          <div className="bg-white dark:bg-slate-900/50 rounded-2xl border border-gray-200 dark:border-slate-800 overflow-hidden transition-colors">
            <button
              ...
              className="w-full px-4 py-3 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-slate-800 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 min-tap"
            >
              <div className="flex items-center gap-3">
                ...
                <div className="text-left">
                  <p className="text-sm font-semibold text-gray-800 dark:text-gray-200">Manage Categories</p>
                  <p className="text-xs text-gray-500 dark:text-slate-400">{categories.length} {categories.length === 1 ? 'category' : 'categories'}</p>
                </div>
              </div>
            ...
            {showCategories && (
              <div className="border-t border-gray-100 dark:border-slate-800 p-4 flex flex-col gap-2">
                {error && (
                ...
                {categories.length === 0 && !isAdding && (
                  <div className="text-sm text-gray-500 dark:text-slate-500 text-center py-5 border border-gray-100 dark:border-slate-800 rounded-xl border-dashed">No categories yet.</div>
                )}

                {categories.map((cat) => {
                  const palette = getCategoryColor(cat.id, cat.color);
                  return (
                    <div key={cat.id} className="bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl group transition-colors">
                      {editingId === cat.id ? (
                        <div className="p-3 flex flex-col gap-2">
                          <Input label="Rename Category" autoFocus value={editName} onChange={e => setEditName(e.target.value)} className="h-8 text-sm !mt-0 dark:bg-slate-900 dark:text-white" onKeyDown={e => e.key === 'Enter' && saveEdit(cat)} />
                          <div>
                            <p className="text-xs font-semibold text-gray-500 dark:text-slate-400 mb-1">Color</p>
                            <ColorPicker value={editColor} onChange={setEditColor} />
                          </div>
                          <div className="flex gap-2 mt-1">
                            <Button size="sm" onClick={() => saveEdit(cat)} disabled={loading} className="flex-1 h-8">Save</Button>
                            <Button size="sm" variant="ghost" onClick={cancelEdit} disabled={loading} className="h-8 px-3 text-gray-500 dark:text-slate-400">Cancel</Button>
                          </div>
                        </div>
                      ) : (
                        <div className="px-3 py-2.5 flex items-center justify-between min-h-[48px]">
                          <div className="flex items-center gap-2 flex-1 min-w-0">
                            ...
                            <span className={`text-sm font-semibold px-2 py-0.5 rounded-md border ${palette.bg} ${palette.text} ${palette.border} dark:bg-opacity-20 dark:border-opacity-30 truncate`}>{cat.name}</span>
                          </div>
                          <div className="flex items-center gap-1 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity ml-2 shrink-0">
                            <button onClick={() => startEdit(cat)} className="p-1.5 text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-slate-700 rounded-md transition-colors min-tap" aria-label="Edit">
                            ...
                            <button onClick={() => deleteCategory(cat.id)} className="p-1.5 text-gray-400 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-slate-700 rounded-md transition-colors min-tap" aria-label="Delete">
                ...
                {isAdding ? (
                  <div className="bg-indigo-50/50 dark:bg-indigo-900/20 border border-indigo-200 dark:border-indigo-800 rounded-xl p-3 flex flex-col gap-2">
                    <Input label="Category Name" autoFocus placeholder="e.g. Furniture" value={newName} onChange={e => setNewName(e.target.value)} className="h-8 text-sm !mt-0 dark:bg-slate-900 dark:text-white" onKeyDown={e => e.key === 'Enter' && saveNew()} />
                    <div>
                      <p className="text-xs font-semibold text-gray-500 dark:text-slate-400 mb-1">Pick a color</p>
                      <ColorPicker value={newColor} onChange={setNewColor} />
                    </div>
                    <div className="flex gap-2 mt-1">
                      <Button size="sm" onClick={saveNew} disabled={loading} className="flex-1 h-8">Add Category</Button>
                      <Button size="sm" variant="ghost" onClick={() => { setIsAdding(false); setNewName(''); setNewColor('violet'); }} className="h-8 px-3 text-gray-500 dark:text-slate-400">Cancel</Button>
                    </div>
                  </div>
                ) : (
                  <button
                    onClick={() => { setIsAdding(true); setError(null); }}
                    className="w-full h-10 border-2 border-dashed border-gray-200 dark:border-slate-700 rounded-xl text-sm font-semibold text-indigo-600 dark:text-indigo-400 hover:border-indigo-300 dark:hover:border-indigo-500 hover:bg-indigo-50/50 dark:hover:bg-indigo-900/20 transition-colors min-tap flex items-center justify-center gap-1.5"
                  >

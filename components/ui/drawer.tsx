...
export function Drawer({ isOpen, onClose, children, title }: DrawerProps) {
  ...
  return createPortal(
    ...
    >
      ...
      <div
        className={cn(
          'relative w-full max-w-2xl mx-auto bg-card rounded-t-3xl shadow-2xl flex flex-col transition-transform duration-300 ease-out max-h-[90vh]',
      ...
      >
        ...
        {title && (
          <div className="flex items-center justify-between px-6 pb-2 border-b border-border shrink-0">
            ...
            <button
              onClick={onClose}
              className="w-8 h-8 flex items-center justify-center rounded-full bg-muted text-muted-foreground hover:bg-muted/80 transition-colors focus:outline-none min-tap"

...
phase: 4
wave: 2
title: Add & Edit Order Form Sheet
depends_on: [01, 02]
files_modified:
  - components/dashboard/order-form-sheet.tsx
  - app/(dashboard)/dashboard/DashboardClient.tsx
autonomous: true
requirements_addressed: [ORD-04, ORD-05, ORD-06, ORD-07, ORD-08, ORD-09, ORD-10]
---

# Plan 03: Add & Edit Order Form Sheet

## Objective

Build a comprehensive Add/Edit Order Form within the Drawer component. It must include photo upload to Supabase Storage, inline-category creation, and mobile-friendly inputs.
...
### Task 3.1: Create Add/Edit Form Component

<action>
Create `components/dashboard/order-form-sheet.tsx`:
  ...
  const handleCategoryChange = async (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (val === 'NEW_CATEGORY') {
      ...
      try {
        ...
        // The real-time hook will pick this up on DashboardClient,
        // but we locally set our categoryId right away.
...
<acceptance_criteria>
- File upload targets `order-photos` with random names
- Form correctly resets state when `isOpen` changes
- Category inline add runs the `window.prompt` then Supabase insert
- Real-time dashboard updates immediately upon submit
</acceptance_criteria>
...
```
## must_haves
- [ ] Photo upload supports mobile camera via `accept="image/*"`
- [ ] Status acts as a segmented control UI
- [ ] FAB on dashboard opens this component in add mode

...
phase: 4
wave: 2
title: Order Detail Bottom Sheet
depends_on: [01]
files_modified:
  - components/dashboard/order-detail-sheet.tsx
  - app/(dashboard)/dashboard/DashboardClient.tsx
autonomous: true
requirements_addressed: [ORD-01, ORD-02, ORD-03]
---

# Plan 02: Order Detail Bottom Sheet

## Objective

Build the Order Detail bottom sheet that displays all fields of a specific order in a read-only format, allows admins to see "Edit" and "Delete" actions, and supports viewing the attached photo in full-screen.
...
### Task 2.1: Create Order Detail Component

<action>
Create `components/dashboard/order-detail-sheet.tsx`:
      ...
      <Drawer isOpen={isOpen && !photoExpanded} onClose={onClose} title={`Order: ${order.order_no}`}>
        <div className="flex flex-col gap-5 pt-2">
          ...
          {order.photo_url && (
            ...
            >
              {/* Using native img for pure client component ease, ideally Next Image but domains unknown */}
              <img 
                src={order.photo_url} 
...
<acceptance_criteria>
- Displays order details neatly
- Tappable photo expands full-screen and closes on tap
- Admin controls (Edit/Delete) only visible to admins
- Delete triggers Supabase DB delete instantly and closes sheet.
</acceptance_criteria>

---

### Task 2.2: Integrate Detail Sheet into Dashboard

<action>
Update `app/(dashboard)/dashboard/DashboardClient.tsx`:
Add state for `selectedOrder` and pass `onClick` down through `OrderCard`.

Wait, we will use `replace_file_content` in Execution to inject `selectedOrder`, `setSelectedOrder`, and render `<OrderDetailSheet>` right before `<OrderFormSheet>`.
</action>

<acceptance_criteria>
- Clicking an order sets `selectedOrder` state
- Sets `isOpen={!!selectedOrder}` on the sheet
</acceptance_criteria>

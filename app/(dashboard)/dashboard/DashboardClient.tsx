...
export function DashboardClient({ user }: { user: AuthUser }) {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-6 pb-24 lg:pb-8" ref={dashboardRef}>
          ...
          <div className="relative">
            ...
            {isExportMenuOpen && (
              <>
                ...
                />
                <div className="bg-card absolute right-0 top-full mt-1 min-w-[130px] w-max border border-border rounded-xl shadow-lg z-50 p-1 flex flex-col animate-in fade-in zoom-in-95 duration-100 hidden-from-print">
                  <button
                    onClick={async () => {
      ...
      {isSelectMode && (
        <div ref={actionBarRef} className="bg-background fixed bottom-0 left-0 right-0 z-50 border-t border-border shadow-2xl px-4 py-3 flex items-center gap-3 animate-in slide-in-from-bottom-2 duration-200">
            ...
            {showFabMenu && (
              <div className="flex flex-col gap-1.5 items-end animate-in fade-in zoom-in-95 duration-150">
                <button
                  ...
                  className="bg-card flex items-center gap-2.5 text-foreground border border-border rounded-2xl shadow-lg px-4 py-2.5 text-sm font-semibold hover:bg-muted/80 transition-colors"
                >
